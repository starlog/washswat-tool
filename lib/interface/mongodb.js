...
const REDIS_KEY_PREFIX = 'wavve-tool-mongodb';
...
exports.ObjectID = function (idString)
{
...
exports.init = function (configurations, callback)
{
    ...
    async.parallel(jobs, function (err, result)
    {
        ...
        {
            logger.info('mongodb initialize success. connection list is ' + JSON.stringify(result));
            callback(null);
        }
    });
};
///////////////////////////////////////////////////////////////////////////////////////////////////
exports.count = function(queryObject, callback)
{
    _count(queryObject.name, queryObject.db, queryObject.collection, queryObject.query, queryObject.sort,
        queryObject.fields, queryObject.skip, queryObject.limit, function (err, result)
        {
            if (!util2.isProduction())
            {
                logger.debug('mongo.count' + util2.debugDump(result, 3, 200));
            }
            callback(err, result);
        });
}
...
{
    _updateOne(queryObject.name, queryObject.db, queryObject.collection, queryObject.query, queryObject.newValue, queryObject.upsert,
        function (err,result)
        {
            if (!util2.isProduction())
            {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
function _count(name, db, collection, query, sort, fields, skip, limit, callback)
{
    ...
    {
        mongoObject.client.db(db).collection(collection).find(query).count(function(err,docs){
            callback(err,docs);
        });
    }
}
...
function _deleteOne(name, db, collection, query, callback)
{
    ...
    {
        mongoObject.client.db(db).collection(collection).deleteOne(query, function (err,result)
        {
            callback(err,result);
        });
    }
...
function _deleteMany(name, db, collection, query, callback)
{
    ...
    {
        mongoObject.client.db(db).collection(collection).deleteMany(query, function (err,result)
        {
            callback(err,result);
        });
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
function _updateOne(name, db, collection, query, newValue, upsert, callback)
{
    let mongoObject = getMongoClient(name);
    ...
    else
    {
        ...
        mongoObject.client.db(db).collection(collection).updateOne(query, newValue, {upsert: isUpsert}, function (err,result)
        {
            callback(err,result);
        });
    }
...
function _insertOne(name, db, collection, newValue, callback)
{
    ...
    {
        mongoObject.client.db(db).collection(collection).insertOne(newValue, function (err,result)
        {
            callback(err,result);
        });
    }
...
function _insertMany(name, db, collection, newValue, callback)
{
    ...
    {
        mongoObject.client.db(db).collection(collection).insertMany(newValue, function (err,result)
        {
            callback(err,result);
        });
    }
